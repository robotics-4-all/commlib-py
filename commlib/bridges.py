...
class RPCBridge(Bridge):
    ...
    def __init__(self, msg_type: RPCMessage = None, *args, **kwargs):
        ...
        self._client = endpoint_factory(EndpointType.RPCClient, to_transport)(
    ...
    def run(self):
        """run."""
        self._server.run()
        self.log.info(f"Started RPC B2B Bridge <{self._from_uri} -> {self._to_uri}")
...
class TopicBridge(Bridge):
    ...
    def __init__(self, msg_type: PubSubMessage = None, *args, **kwargs):
        ...
        self._pub = endpoint_factory(EndpointType.Publisher, to_transport)(
            topic=self._to_uri,
            msg_type=self._msg_type,
            conn_params=self._to_broker_params,
        )
...
class PTopicBridge(Bridge):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._pub = endpoint_factory(EndpointType.MPublisher, to_transport)(
            msg_type=self._msg_type,
            conn_params=self._to_broker_params,
        )
    ...
    def run(self):
        ...
        self._sub.run()
        self.log.info(
            "Started B2B Multi-Topic Bridge "
            + f"<{self._from_broker_params.host}:"
            + f"{self._from_broker_params.port}[{self._from_uri}] "
            + f"-> {self._to_broker_params.host}:"
            + f"{self._to_broker_params.port}[{self._to_uri}.*]>"
