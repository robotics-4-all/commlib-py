...
class RealTimePlotter:
    ...
    def update(self, frame):
        ...
        # Scatter plot
        self.sc = self.ax.scatter(self.xs, self.ys, self.zs, c=self.cs, cmap='viridis', marker='o')
...
class PubSubPerfTest:
    ...
    def run_single_size(self, num_subs: int, size_kb: int, target_freq: int, num_messages: int, node: Node, pub: Any):
    ...
    def _worker_run_subs(self, num_subs: int, sizes_list: List[int], freqs_list: List[int], num_messages: int, results_queue: multiprocessing.Queue):
        for size_kb in sizes_list:
            ...
            for target_freq in freqs_list:
                res = self.run_single_size(num_subs, size_kb, target_freq, num_messages, node, pub)
    ...
    def run_benchmark(self, subs_list: List[int], sizes_list: List[int], freqs_list: List[int], num_messages: int, plot_queue: multiprocessing.Queue = None):
        results = []
        ...
        with Progress(
            ...
            for num_subs in subs_list:
                ...
                process = multiprocessing.Process(
                    ...
                    args=(num_subs, sizes_list, freqs_list, num_messages, results_queue)
                )
                process.start()
                ...
                while True:
                    ...
                    if plot_queue:
                        plot_queue.put(res)
                    progress.update(overall_task, advance=1, description=f"[cyan]Subs={res['num_subs']}, Size={res['data_size_kb']}KB, Freq={res['target_freq']}Hz")
...
def main():
    ...
    parser.add_argument("--num-messages", type=int, default=100, help="Number of messages per data size")
    ...
    parser.add_argument("--max-subs", type=int, default=16, help="Maximum number of subscribers to test")
    ...
    try:
        results = tester.run_benchmark(subs_list, sizes_list, freqs_list, args.num_messages, plot_queue)
    finally:
        if plot_process:
