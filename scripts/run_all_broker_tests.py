...
def wait_for_healthy(service, progress, task_id, timeout=60):
    ...
    while time.time() - start_time < timeout:
        rc, stdout, stderr = run_command(f"docker-compose -f {COMPOSE_FILE} ps {service} --format json")
        if rc == 0 and stdout:
...
def main():
    ...
    for service, broker_type in SERVICES.items():
        console.print(f"\n[bold yellow]Testing Service:[/bold yellow] [bold cyan]{service}[/bold cyan] [dim]({broker_type})[/dim]")
        ...
        with Progress(
            ...
            rc, stdout, stderr = run_command(f"docker-compose -f {COMPOSE_FILE} up -d {service}")
            progress.update(start_task, completed=10)
            
            if rc != 0:
                console.print(f"[bold red]✘ Failed to start {service}:[/bold red] {stderr}")
                results.append({
                    "service": service,
                    "broker": broker_type,
                    "status": "FAILED (Start)",
            ...
            # Wait for Health
            health_task = progress.add_task(f"Waiting for {service} health...", total=60)
            if not wait_for_healthy(service, progress, health_task):
                console.print(f"[bold orange3]⚠ Service {service} health check timed out, proceeding anyway...[/bold orange3]")
            ...
            for script in TEST_SCRIPTS:
                ...
                if rc == 0 and "SUCCESS" in stdout and "FAILURE" not in stdout:
                    service_results.append((script_name, "[bold green]PASSED[/bold green]"))
                else:
            ...
            results.append({
                "service": service,
                "broker": broker_type,
                "status": "[bold green]PASSED[/bold green]" if all_passed else "[bold red]FAILED[/bold red]",
    ...
    # Final Report
    console.print("\n")
    table = Table(title="[bold blue]Final Test Report[/bold blue]", box=box.ROUNDED, show_header=True, header_style="bold magenta")
    ...
    for res in results:
        ...
        test_details = ""
        if "tests" in res:
            test_details = "\n".join([f"• {name}: {stat}" for name, stat in res["tests"]])
        elif "details" in res:
    ...
    console.print(Panel(
        f"[{summary_color}]Summary: {total_passed}/{len(SERVICES)} services passed all tests.[/{summary_color}]",
        box=box.ROUNDED,
        expand=False
    ))
