...
def main():
    ...
    parser.add_argument("--broker", type=str, default="redis", choices=["redis", "amqp", "mqtt", "kafka"], help="Broker type")
    parser.add_argument("--action-name", type=str, default="test.action.example", help="Action name to use")
    parser.add_argument("--target", type=int, default=5, help="Target value for the action")
    ...
    try:
        ...
        if result is None:
            ...
            if result.final_pos == args.target:
                ...
                print(f"FAILURE: Expected final_pos={args.target}, got {result.final_pos}")
            ...
            if feedback_received[-1] == args.target:
                ...
                print(f"FAILURE: Last feedback {feedback_received[-1]} does not match target {args.target}")
